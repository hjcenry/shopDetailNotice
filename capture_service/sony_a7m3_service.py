...
class SonyA7M3Service(object):
    item_keys = ['sony_a7m3', 'sony_a7m3k']
    config = None
    redis_dao = None

    item_min_price_key_pre = 'min_price_'
    item_sold_state_key_pre = 'sold_state_'
            ...
            for item_key in self.item_keys:
                ...
                if item_key in platform_keys:
                    ...
                    for key in platform_keys[item_key].split(','):
                        ...
                        # 记录到货通知
                        # FIXME
                        # if shop_detail_capture.is_item_sold_out(result_json):
                        #     sold_out_notice = shop_detail_capture.get_item_sold_out_text(key)
                        #     print(sold_out_notice)
                        #     # 存储redis
                        #     sold_state_str = self.redis_dao.get_value(self.item_sold_state_key_pre + item_key)
                        #     if sold_state_str is None or sold_state_str == str(False):
                        #         self.redis_dao.set_value(self.item_sold_state_key_pre + item_key, str(True))
                        #         self.msg_notice(item_name, " 无货了 ", sold_out_notice)
                        # else:
                        #     # 存储redis
                        #     sold_state_str = self.redis_dao.get_value(self.item_sold_state_key_pre + item_key)
                        #     if sold_state_str is None or sold_state_str == str(True):
                        #         self.redis_dao.set_value(self.item_sold_state_key_pre + item_key, str(False))
                        #         self.msg_notice(item_name, " 到货了 ",)
